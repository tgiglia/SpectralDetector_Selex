    ...
    float ref_lvl)
{
    ...
    AnalyzeRelativeGain aRg(frame.get_plot_w(),10,13.0);
    ...
    bool isJammerFrame = aRg.IsFrameAJammer();
    aRg.logWithTime("ARG.txt","Processing Frame...");
    aRg.logIt("ARG.txt",aRg.showObj());
...
std::string scan_for_jammer(const log_pwr_dft_type& dft_,
    size_t width,
    size_t height,
    double samp_rate,
    double dc_freq,
    ...
    AnalyzeRelativeGain aRg(frame.get_plot_w(),num_jammer_bins,jammerThreshold);
    // fill the plot with dft bins
    for (size_t b = 0; b < frame.get_plot_w(); b++) {
                // indexes from the dft to grab for the plot
        const size_t n_start = std::max(
    ...
    bool isJammerFrame = aRg.IsFrameAJammer();
    aRg.logWithTime(jammerDbgFile,"Processing Frame...");
